...
def _flash_attention_kernel(
    ...
    *, mask_value: float, grid_width: int, bq: int, bkv: int,
    bkv_compute: int, bkv_compute_in: int, head_dim_v: int,
    ...
    def compute_body(kv_compute_index, _):
        ...
        # OPTIMIZATION 1: Keep V in native BF16!
        v = v_ref[slice_k, :] 
        ...
        for i in range(0, qk.shape[0], step):
            ...
            # OPTIMIZATION 2: Native jnp.exp2 instruction
            ...
            # OPTIMIZATION 3: Cast Softmax weights to BF16 for the fast MXU path!
            s_curr_bf16 = s_curr.astype(bfloat16)
            
            sv_dims = (((0,), (0,)), ((), ()))
            # Execute dot product with BF16 inputs, accumulating into FP32
            o_curr = lax.dot_general(v[i : i + step], s_curr_bf16, sv_dims, preferred_element_type=float32)
    ...
    def last_compute_body(kv_compute_index):
        ...
        sv_dims = (((0,), (0,)), ((), ()))
        o_curr = lax.dot_general(v, s_curr_bf16, sv_dims, preferred_element_type=float32)
...
def make_splash_mha(block_sizes: _BlockSizes, bkv_compute_in: int, interpret: bool = False):
    def _splash_attention(q: jax.Array, k: jax.Array, v: jax.Array):
        return __splash_attention_forward(q, k, v, block_sizes, bkv_compute_in, interpret)
...
def _tpu_splash_attention(query, key, value, mesh):
    ...
    def _attention_on_slices(q, k, v):
        ...
        def kernel_3d(q_3d, k_3d, v_3d):
            ...
            # OPTIMIZATION 4: Fixed edge-case crash for small dummy sequences
            bkv_comp = min(BKVCOMPUTESIZE, padded_kv_seq_len)
            
            block_sizes = _BlockSizes(
                block_q=min(BQSIZE, padded_q_seq_len),
                block_kv=min(BKVSIZE, padded_kv_seq_len),
                block_kv_compute=bkv_comp,
            )
            splash_kernel = make_splash_mha(block_sizes=block_sizes, bkv_compute_in=bkv_comp)
